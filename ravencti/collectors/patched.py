"""
collectors/patched.py — patched.to collector.

Platform: patched.to (MyBB forum, "Ashen" custom theme)
...
URL patterns:
  ...
  - Thread view:         /Thread-{slug}
  - Search:              /search.php?action=do_search&keywords=...

Auth:
  Set PATCHED_COOKIES in .env.
  Get cookies from browser DevTools after logging into patched.to.
...
from ravencti.config import PATCHED_COOKIES, MONITORED_KEYWORDS, MONITORED_DOMAIN
...
log = logging.getLogger("ravencti.collectors.patched")

_SOURCE = "patched"
_BASE_URL = "https://patched.to"
...
def _classify(text):
    ...
    if any(w in tl for w in ["ransomware", "encrypt", "locked"]):
...
def _extract_threads(html):
    ...
    if not threads:
        for m in _RE_THREAD.finditer(html):
            href = m.group(1)
            if "?action=" in href or href in seen:
                continue
...
def _parse_thread_content(html, keyword):
    """Parse a single thread page for full content."""
    title = ""
    ...
    url_match = re.search(r'href="(https?://patched\.to/Thread-[^"?#]+)"', html, re.IGNORECASE)
    url = url_match.group(1) if url_match else ""

    return {
        "title": f"[Patched] {prefix} {title}".strip()[:200],
...
def collect_patched():
    jid = job_start("patched_intel")
    try:
        session = get_session()
        has_cookies = _apply_cookies(session, PATCHED_COOKIES)
        n = 0

        with get_db() as conn:
            threads_seen = set()

            r = safe_get(f"{_BASE_URL}/", session=session, timeout=25)
            if r is not None and r.status_code == 200:
                for m in _RE_FID_LINK.finditer(r.text):
                    fid = int(m.group(1))
            ...
            for keyword in MONITORED_KEYWORDS:
                try:
                    ...
                    log.debug("[PATCHED] Search error for '%s': %s", keyword, e)
                    time.sleep(5)

            if has_cookies and threads_seen:
                scan_list = list(threads_seen)[:50]
                log.info("[PATCHED] Deep-scanning %d threads with cookies", len(scan_list))
                for url in scan_list:
                    try:
                        r = safe_get(url, session=session, timeout=25)
                        if r is None or r.status_code != 200:
                            continue
                        ...
                        log.debug("[PATCHED] Thread error: %s", e)
                        time.sleep(3)

        update_source("patched_monitor", "success", n)
        job_done(jid, "completed", n)
        if n > 0:
            log.info("[PATCHED] %d new findings (cookies=%s)", n, has_cookies)
            try:
            ...
            log.warning("[PATCHED] 0 findings")

    except Exception as e:
        update_source("patched_monitor", "error")
        job_done(jid, "failed", 0, str(e))
        log.exception("collect_patched failed")
